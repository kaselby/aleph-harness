...
Usage:
    python -m aleph.tui.channels <channel-name> [--as <user>] [--home <path>]
"""
...
def format_message(msg: dict, show_body: bool = False) -> str:
    ...
    # Agents have IDs like "aleph-xxx" or "ts-xxx"; users have simple names
...
class ChannelViewer:
    def __init__(self, channel: str, user: str = "kira", home: Path | None = None):
        ...
        self.user = user
        self.home = home or Path(os.environ.get("ALEPH_HOME", Path.home() / ".aleph"))
    ...
    def _viewer_id(self) -> str:
        """Pseudo-subscriber ID for this viewer instance."""
        return f"viewer-{self.user}"

    def read_subscribers(self) -> list[str]:
        """Read channel subscribers from channels.json."""
    ...
    def _subscribe(self) -> None:
        """Register the viewer as a channel subscriber so agents can broadcast to it."""
    ...
    def _unsubscribe(self) -> None:
        """Remove the viewer from channel subscribers on exit."""
        if not self.channels_path.exists():
            return
        try:
            with open(self.channels_path, "a+") as f:
    ...
    def send_message(self, text: str) -> None:
        """Send a message to the channel: append to history + deliver to subscribers."""
        now = datetime.now(timezone.utc)
        # Short messages: summary only. Long messages: truncated summary + full body.
        if len(text) <= 200:
            summary, body = text, ""
        else:
        ...
        # Deliver to subscriber inboxes (skip self and viewer pseudo-subscribers)
    ...
    def _deliver_to_inbox(self, recipient: str, summary: str, body: str, now: datetime) -> None:
        """Write a message file to a subscriber's inbox."""
    ...
    def _read_new_lines(self) -> list[dict]:
        """Read new lines from the JSONL file since last check."""
    ...
    def _print_msg(self, msg: dict) -> None:
        """Print a single formatted message."""
    ...
    async def _tail(self) -> None:
        """Background task: poll JSONL for new messages."""
        while self._running:
            try:
                for msg in self._read_new_lines():
                    # Skip own messages — already echoed by the prompt
    ...
    async def run(self) -> None:
        """Main loop: print history, start tailing, accept input."""
        # Register as subscriber so agent broadcasts reach the JSONL
        self._subscribe()
...
def main():
    parser = argparse.ArgumentParser(
        description="Aleph channel viewer",
        usage="python -m aleph.tui.channels CHANNEL [--as USER] [--home PATH]",
    )
    ...
    parser.add_argument(
        ...
        help="Aleph home directory (default: ~/.aleph)",
    )
